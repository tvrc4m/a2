    ...
    <div class="container">
        ...
        <div class="row">
            <form class="form-horizontal" role="form">
                ...
                <iform type="texteare" label="介绍" placeholder="请输入介绍" required :value.sync="company.content"></iform>
                <iform type="text" label="排序" placeholder="请输入排序值" required :value.sync="company.sort"></iform>
        ...
        methods:{
            uploadImg(file){
                uploadImage(file).then(data=>{
                    this.$set(this.company, 'img', data.code)
                })
            },
            updateCompany(){
                if(!assertEmpty(this.company.realname)){
                    this.$message.error("请选择姓名")
                    return false
                }
                
                if(!assertEmpty(this.company.job)){
                    this.$message.error("请输入职务")
                    return false
                }
                if(!assertEmpty(this.company.phone)){
                    this.$message.error("请输入手机号")
                    return false
                }
                if(!assertPhone(this.company.phone)){
                    this.$message.error("手机号不正确")
                    return false
                }
                this.company.stype = 2
                if(this.add){
